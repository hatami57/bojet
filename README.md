...
func main() {
	st, err := store.NewStore("./bot.db") // SQLite-backed UserStore
	if err != nil {
		log.Fatal(err)
	}
	defer st.Close()
	...
	bot, err := bojet.New("YOUR_BOT_TOKEN",
		bojet.WithStore(st),
...
bojet.New(token, bojet.WithStore(st), bojet.WithAdmins(123456789), ...)
...
bojet.New(token, bojet.WithStore(st), bojet.WithPublicAccess(), ...)
```
...
- `bot.Broadcast(msg)` — send to all confirmed users.
- `bot.Schedule(cronExpr, fn)` / `bot.ScheduleBroadcast(cronExpr, msg)`.
- Lifecycle hooks: `OnUserRegistered`, `OnUserApproved`, `OnUserRejected`.
- `WithProxy`, `WithPollTimeout`, `WithCacheExpiry`, `WithLogger`, and
  config-file loading via `LoadConfig` / `NewFromConfig`.
